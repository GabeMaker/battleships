...
describe Player do
  ...
  let (:batttleship){double :batttleship}
  
  it 'can place a ship on own board horizontally' do
    player = Player.new
    player.add_board(board1)
    expect(board1).to receive(:place_horizontal).with("a1", batttleship)
    player.place(batttleship, "a1", :horizontal)
	end

  it 'can place a ship on own board vertically' do
    player = Player.new
    player.add_board(board1)
    expect(board1).to receive(:place_vertical).with("a1", batttleship)
